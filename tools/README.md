...
個別:
```bash
uv run python tools/gen_01_setup_and_data.py notebooks/01_setup_and_data.ipynb full   # 完成版
uv run python tools/gen_01_setup_and_data.py notebooks/01_setup_and_data.ipynb blank  # 穴埋め版
```
一括:
```bash
...
uv run jupyter nbconvert --to notebook --execute --inplace \
  --ExecutePreprocessor.timeout=1200 notebooks/03_train_wandb.ipynb
```
