...
> 【暫定・要確定】この予習節は **第1回（data）の当日内容が fix してから確定する**．
> 予習＝第1回の下ごしらえなので，第1回で「可変長の波形をどう束ねるか（`collate_fn`）／ラベルの index 化を当日どこまでやるか／何を予習に回すか」が決まって初めて，予習の問いを過不足なく決められる．
> 下記は現時点の暫定版（第1回でつまずく定番＝可変長の束ね方・ラベルの扱いを先回りさせる方向）．
